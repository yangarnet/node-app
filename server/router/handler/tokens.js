                ...
                if (!err && data) {
                    ...
                    if (hashedPassword === data.password) {
                        ...
                        _data.create('tokens', tokenId, token, err => {
                            if (!err) {
                                callback(200, {succes: 'create new token success'});
                            } else {
                                callback(400, {errror: 'creating token error'});
                            }
                        });
                    } else {
                        callback(400, {error: 'user password does not match'});
                    }
                } else {
                    callback(err, data);
                }
                ...
                if(!err && data) {
                    if (data.expires > Date.now()) {
                        ...
                        _data.update('tokens', tokenId, data, (err, res) => {
                            if(!err && res) {
                                callback(200, {succes: 'extend token successful'});
                            } else {
                                callback(500, {error: 'cannot extend the token'});
                            }
                        });
                    } else {
                    ...
                    callback(404, {error: `cannot find token by id ${tokenId}`});
                }
            });
        } else {
            callback(400, {error: 'missing required tokenid and extend=true to update token'});
        }
    },
            ...
            _data.read('tokens', tokenId, (err, res) => {
                if (!err && res) {
                    _data.delete('tokens', tokenId, (err, response) => {
                        if (!err && response) {
                            callback(200, {succes: `delete token success: ${tokenId}`});
                        } else {
